...
export class Albums extends Component { 
    ...
    albums = [
      ...
      handleDelete(id) {
        this.setState({
          albums: this.state.albums.filter(el => {
            return el.id !== id
          })
        })
      };
      
      // Handle addition of a new album
      handleAddAlbum(newAlbum) {
        const updatedAlbums = [...this.state.albums, newAlbum];
        this.setState({
      ...
      handleEditAlbum(updatedAlbum) {
        const updatedAlbums = this.state.albums.map(album => {
          if (album.id === updatedAlbum.id) {
            // Update the title and userId of the matched album
            return {
              ...album,
    ...
    return (
      <div className="container my-3">
        <div className="heading">
          <h2><FaMusic className="music-icon" /> Albums List</h2>
        </div>
          {/* ModalDialog component for adding albums */}
        <ModalDialog buttonText="Add Album" buttonName="Add" onClick={(e) => this.create(e)} 
        onAddAlbum={newAlbum => this.handleAddAlbum(newAlbum)} 
        />
       
        <div className='row'>
             {/* Loop through albums and render AlbumItems for each */}
        {this.state.albums.map((element)=>{
              return <div className="col-md-3"  key={element.id}>
              <AlbumItems title={element.title.slice(0,17)} userId={element.userId} id = {element.id} onhandleDelete = {this.handleDelete}  onhandleEditAlbum={this.handleEditAlbum} />
              </div>
